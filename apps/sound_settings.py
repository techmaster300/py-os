...
class ThemeCreatorApp(BlindApp):
    ...
    def run(self):
        ...
        create_btn = wx.Button(panel, label="Create New Theme")
        create_btn.SetName("Create New Theme")
        create_btn.Bind(wx.EVT_BUTTON, lambda evt: self.start_create())
        sizer.Add(create_btn, 0, wx.EXPAND | wx.ALL, 10)

        manage_btn = wx.Button(panel, label="Manage Custom Themes")
        manage_btn.SetName("Manage Custom Themes")
        manage_btn.Bind(wx.EVT_BUTTON, lambda evt: self.start_manage())
        sizer.Add(manage_btn, 0, wx.EXPAND | wx.ALL, 10)

        self.theme_name_input = wx.TextCtrl(panel, style=wx.TE_PROCESS_ENTER)
        self.theme_name_input.SetName("Theme Name")
        self.theme_name_input.Hide()

        self.mode_choice = wx.Choice(panel, choices=["Tones", "Audio File"])
        self.mode_choice.SetName("Sound Mode")
        ...
        self.manage_btn_row = wx.BoxSizer(wx.HORIZONTAL)
        self.rename_btn = wx.Button(panel, label="Rename")
        self.rename_btn.SetName("Rename Theme")
        self.rename_btn.Bind(wx.EVT_BUTTON, self.on_rename_theme)
        self.delete_btn = wx.Button(panel, label="Delete")
        self.delete_btn.SetName("Delete Theme")
        self.delete_btn.Bind(wx.EVT_BUTTON, self.on_delete_theme)
        self.apply_btn = wx.Button(panel, label="Apply")
        self.apply_btn.SetName("Apply Theme")
        self.apply_btn.Bind(wx.EVT_BUTTON, self.on_apply_theme)
